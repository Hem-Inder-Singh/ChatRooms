...
//        return super.serve(uri, method, header, parms, files); //To change body of generated methods, choose Tools | Templates.
    ...
    else if(uri.contains("/GetResource"))
    {
       uri = uri.substring(1);
        ...
        System.out.println("check 1.."+uri);
    ...
    else if(uri.contains("/Delete_room"))
    {
        ...
        try {
            ...
            {
                rs.deleteRow();
                res=new Response(HTTP_OK,"text/plain","Row Deletion Success");
            }
        } catch (Exception ex) {
            Logger.getLogger(chat_room_server.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    else if(uri.contains("/user_signup"))
    {
        ...
        String password=parms.getProperty("password");
        String mobileno=parms.getProperty("mobile_no");
        String sec_ques=parms.getProperty("sec_ques");
        String sec_ans=parms.getProperty("sec_ans");
       
        String photopath=saveFileOnServerWithRandomName(files, parms, "photo","src/usersprofile");
        String ans;
        try {
            ...
            else
            {
                ...
                rs.updateString("photo","src/usersprofile/"+photopath);
                rs.insertRow();
                ans="Success";
            }
             res=new Response(HTTP_OK,"text/plain",ans);
    ...
    else if(uri.contains("/user_forget_password"))
    {
        String username=parms.getProperty("username");
        ...
        String ans=parms.getProperty("ans");
         String ans2;
        try {
            ResultSet rs = DBLoader.executeQuery("select * from users where username='"+username+"' and sec_ques='"+ques+"' and sec_ans='"+ans+"'");
            if(rs.next())
            {
